...
fn trait_constraints_on_structs() {
    ...
    impl<T: Ord> IsStable for Language<T> {
        fn is_stable(&self) -> bool {
...
fn inheritance() {
    ...
    impl<K, V> PartialEq for MyHashMap<K, V> {
        fn eq(&self, other: &Self) -> bool {
            false
        }
    }
    impl<K, V> PartialOrd for MyHashMap<K, V> {
    ...
    impl<K, V> Ordering for MyHashMap<K, V> {
        fn is_before(&self, other: &Self) -> bool {
            self < other
        }
    }
    ...
    let a: MyHashMap<u8, u8> = MyHashMap(HashMap::new());
    let b = MyHashMap(HashMap::new());
